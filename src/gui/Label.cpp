...
Label::Label(sf::RenderWindow *window, sf::Font *font, std::wstring str, const float x, const float y, Align align, const unsigned int fSize,
             const float w)
...
sf::Text &Label::getText() { return text; }
...
void Label::SetAnimation(Anims type) { this->animType = type; }
...
void Label::AlignToCenter() {
    ...
    for (auto it = text.getString().begin(); it != text.getString().end(); ++it) {
        if (maxWidth > i * (text.getCharacterSize() + text.getLetterSpacing())) {
            ...
            newStr.push_back(*it);
            // DEBUG
            // std::cout << std::string(newStr.begin(), newStr.end()) <<
            // std::endl; std::wcout <<  maxWidth << L' ' << i *
            // text.getCharacterSize() << L' ' << newStr << L'\n';
            ++i;
        } else
    ...
    if (!text.getString().isEmpty()) {
        ...
        text.setOrigin(sf::Vector2f(text.getGlobalBounds().width / 2,
                                    (text.getCharacterSize() - text.getGlobalBounds().height) + text.getGlobalBounds().height / 2));
