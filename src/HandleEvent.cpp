...
{   
    ...
    for (const auto &i : gamew->ObjVector)
    {
        ...
        if (auto *b = dynamic_cast<Button *>(i.get()); b && b->getRect().getGlobalBounds().contains(sf::Vector2f((float)event.mouseMove.x + gamew->screenOffsetX, (float)event.mouseMove.y + gamew->screenOffsetY)))
        {
            b->SetHovered();
        }
        else if (b && b->isHovered() && !b->getRect().getGlobalBounds().contains(sf::Vector2f((float)event.mouseMove.x + gamew->screenOffsetX, (float)event.mouseMove.y + gamew->screenOffsetY)))
            b->ResetHovered();
        else if (auto *b = dynamic_cast<TextBox *>(i.get()); b && b->getRect().getGlobalBounds().contains(sf::Vector2f((float)event.mouseMove.x + gamew->screenOffsetX, (float)event.mouseMove.y + gamew->screenOffsetY)))
        {
...
void HandleEvent::eventMouseButtonPressed(const sf::Event &event)
{
    for (const auto &i : gamew->ObjVector)
    {
        ...
        if (auto *b = dynamic_cast<Button *>(i.get()); b && b->getRect().getGlobalBounds().contains(sf::Vector2f((float)event.mouseButton.x + gamew->screenOffsetX, (float)event.mouseButton.y + gamew->screenOffsetY)))
            gamew->HandleButton(b);
        else if (auto *b = dynamic_cast<TextBox *>(i.get()); b && b->getRect().getGlobalBounds().contains(sf::Vector2f((float)event.mouseButton.x + gamew->screenOffsetX, (float)event.mouseButton.y + gamew->screenOffsetY)))
